    ...
    request.onreadystatechange = function() {
      if (request.readyState === XMLHttpRequest.DONE) {
        if (request.status === 200) {
          ...
          console.log(JSON.stringify(app.currencies.rates));
        }
      } 
    };
  ...
  function onSelect(currency, type) {
  	...
  	document.getElementById('result').innerHTML = app.currencies.rates[currency];
  }

  document.getElementById('fromCurrency').innerHTML = app.fromCurrencyName;
  document.getElementById('toCurrency').innerHTML = app.toCurrencyName;
